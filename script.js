const inputPointNumber = document.getElementById("input_point_number");
	...
	function drowTree(num,ftr){
		...
		for(let i=1;i<num.length;++i){
			let pointT = num.indexOf(i);
			const parent = [...document.getElementsByClassName("parent")];
            const child = [...document.getElementsByClassName("child")];
			let parentT = ftr[pointT];
//			console.log(pointT);
//			console.log(parentT);
//			console.log(vetki);
			if (parentT == 0){
				tree.innerHTML += `<div class="vetka"><div class="parent">${i}</div><div class="child"></div></div>`;
			} else {
				parent.map( (elem, index) =>{
					//console.log(elem.innerText.substr(0,parentT.toString().length))
					if(elem.innerText.substr(0,parentT.toString().length) == parentT){
						child[index].innerHTML += `<div class="vetka"><div class="parent">${i}</div><div class="child"></div></div>`;
					} 
				})
			}
		}	
